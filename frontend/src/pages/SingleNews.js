...
const SingleNews = () => {
  ...
  const text = { textIndent: "50px", textAlign: "justify" };
  let content;
  if (isLoading) {
    content = (
    ...
    content = (
      <div className="card-body px-5">
        ...
        <Link
          to={`/news/employee/${news.employee}`}
          style={{ textDecoration: "none", color: "black" }}
        >
          <h6 className="card-subtitle mb-2 text-muted mt-2">
            {news.username} - Posted: {format(new Date(news.createdAt), "PP")}
          </h6>
        </Link>
        ...
        <button
          ...
          className={`shadow-none btn ${
            news.likes.includes(userId) ? "btn-danger" : "btn-secondary"
          }`}
          onClick={likeBtn}
          disabled={roles.includes(2001) ? false : true}
        >
